...
def pdstsp_dp_drop_pickup(
    c=8, kappa_t=2, kappa_d=3, beta=55, seed=42, solve_time=25, verbose=True
):
    random.seed(seed)
    depot = 0
    ...
    num_customers = len(J)
    num_truck_only = random.randint(
        max(1, int(0.2 * num_customers)), max(1, int(0.4 * num_customers))
    )
    ...
    if drone_eligible_customers:
        drop_customers = set(
            random.sample(
                drone_eligible_customers,
                random.randint(1, len(drone_eligible_customers)),
            )
        )
    ...
    tau_t = {
        (i, j): random.randint(8, 15) + random.randint(0, 1)
        for i in full_nodes
        for j in full_nodes
        if i != j
    }
    # Drone travel times.
    tau_d = {
        (i, j): random.randint(5, 10) for i in full_nodes for j in full_nodes if i != j
    }
    ...
    ItvAlt = {
        t: {
            j: interval_var(
                optional=True, size=service_times[j], name=f"ItvAlt_{t}_{j}"
            )
            for j in J
        }
    ...
    DroneMsnVars = [
        interval_var(optional=True, name=f"DroneMsn_{i}")
    ...
    for d in range(kappa_d):
        ...
        if intervals is not None:
            ...
            for interval in intervals_sorted:
                ...
                if tp == "drop":
                    output_text += (
                        f"  Drone {d}: 0 → {nodes[0]}(Drop) → 0, time [{st},{ed}]\n"
                    )
                elif tp == "drop-pickup":
        ...
        if intervals is not None:
            ...
            if route:
                ...
                for u, v in zip(full_path[:-1], full_path[1:]):
                    G.add_edge(
                        u, v, color="tab:blue", width=2.2, style="solid", label=f"T{t}"
                    )


    drone_color = "tab:green"
    ...
    for d in range(kappa_d):
        ...
        if intervals is not None:
            ...
            for interval in intervals_sorted:
                ...
                G.add_edge(
                    0, nodes[0], color=col, style="solid", label=f"D{d}", width=2.5
                )
                ...
                G.add_edge(
                    nodes[-1], 0, color=col, style="dotted", label=f"D{d}", width=2
                )
    ...
    legend_elements = [
        ...
        Line2D(
            [0], [0], color="tab:green", lw=2, linestyle="-", label="Drone Drop-only"
        ),
        Line2D(
            [0], [0], color="tab:green", lw=2, linestyle="-.", label="Drone Drop→Pickup"
        ),
        Line2D([0], [0], color="tab:green", lw=2, linestyle=":", label="Drone Return"),
    ]
    ...
    plt.title("CP Optimized Routes", fontsize=14)
    plt.axis("off")
    plt.tight_layout()
    buf = io.BytesIO()
